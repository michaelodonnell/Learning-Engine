...
class Database {
    ...
    var $user     = "learning-engine"; //database login name
    var $pass     = "PWSUrhXpNdJqrZ8T"; //database login password
    var $database = "learning-engine"; //database name
    
    var $pre      = ""; //table prefix
    
    // DB Connection Handles:
    var $mainDBHandle = null; // Handle for main DB connection
    
    public $historicalDBAvailable;
    ...
    #-#############################################
    # desc: does a lock query
    # param: table (no prefix), write/read
    public function lock($table, $mode) {
        
        # needed to ammend this to lock multiple tables - RF 25/3/10
    ...
    function oops($msg='') {
        ...
        // Mail the error:
        mail('mail@michaelodonnell.ie', 'PM: MySQL Error', $msg . " - " . $this->error . " - " . @$_SERVER['REQUEST_URI']);
    ...
    function count($sql) {
        ...
        return $this->affected_rows = @mysql_affected_rows($this->link_id);
